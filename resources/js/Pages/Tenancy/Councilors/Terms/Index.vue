...
const form = useForm({
    ...
    start_date: '',
    // 1. CORRIGIDO: O tipo de 'end_date' agora pode ser nulo
...
const openEditModal = (term: Term) => {
    ...
    // 2. CORRIGIDO: Garante que 'end_date' seja string ou nulo
...
<template>
                    ...
                    <div v-if="data.terms.length > 0">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead class="bg-gray-50 dark:bg-gray-700/50">
                                <tr>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Data de Início</th>
                                ...
                                <tr v-for="term in data.terms" :key="term.id">
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex items-center justify-end space-x-1">
                                            <IconButton @click="openEditModal(term)" color="yellow" title="Editar"><PencilSquareIcon class="h-5 w-5" /></IconButton>
                                            <IconButton @click="openConfirmDeleteModal(term)" color="red" title="Excluir"><TrashIcon class="h-5 w-5" /></IconButton>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
    ...
    <ConfirmDeletionModal :show="confirmingDeletion" title="Excluir Mandato" message="Tem certeza que deseja excluir este mandato?" buttonText="Excluir" @close="closeDeleteModal" @confirm="deleteTerm" />
</template>