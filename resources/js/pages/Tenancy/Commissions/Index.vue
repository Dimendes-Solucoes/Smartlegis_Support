...
watch(selectedLegislature, (value) => {
  router.get(
    ...
    { legislature_id: value ?? undefined },
    {
      preserveState: true,
      replace: true,
    }
...
const toggleStatus = (commission: Commission) => {
  const action = commission.is_active ? "inativar" : "ativar";
  if (
    confirm(`Tem certeza que deseja ${action} a comissão "${commission.comission_name}"?`)
  ) {
    router.patch(
      route("commissions.toggle_status", commission.id),
      {},
      {
        preserveScroll: true,
        preserveState: true,
    ...
    <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-4">
      <div class="w-full sm:w-72">
        ...
        <SelectInput
          ...
          :options="props.legislatures"
          value-key="id"
          label-key="title"
          placeholder="Todas as legislaturas"
          class="mt-1 w-full"
        />
      </div>

      <TextButton :href="route('commissions.create')"> Nova Comissão </TextButton>
    </div>
      ...
      <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
        ...
        >
          <tr v-for="commission in props.commissions" :key="commission.id">
            ...
            <td
              class="px-4 py-4 whitespace-nowrap text-sm font-medium flex items-center gap-2"
            >
              <IconButton
              ...
              <IconButton
                ...
                :color="commission.is_active ? 'red' : 'green'"
                :title="commission.is_active ? 'Inativar' : 'Ativar'"
              >
                <NoSymbolIcon v-if="commission.is_active" class="h-5 w-5" />
